...
function renderBuildingConfig() {
    ...
    buildingIds.forEach(id => {
        ...
        card.className = 'property-card group bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col';

        // Using a simple background style
        card.innerHTML = `
                <div class="h-40 bg-gray-100 relative overflow-hidden">
                    <div class="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-110" style="background-image: url('https://picsum.photos/seed/${id}/600/400')"></div>
                    <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                    <div class="absolute bottom-4 left-4 text-white">
                        <h2 class="text-xl font-bold tracking-tight">${building.name}</h2>
                        <p class="text-xs opacity-90 font-mono">ID: ${id.substring(0, 8)}</p>
                    </div>
                </div>
                
                <div class="p-5 flex-grow flex flex-col justify-between">
                    <div class="flex justify-around items-center mb-6 py-3 bg-gray-50 rounded-xl border border-gray-100">
                        <div class="text-center">
                            <span class="block text-xl font-bold text-gray-800">${floorCount}</span>
                            <span class="text-xs text-gray-500 font-medium uppercase tracking-wider">Floors</span>
                        </div>
                        <div class="w-px h-8 bg-gray-200"></div>
                        <div class="text-center">
                            <span class="block text-xl font-bold text-gray-800">${roomCount}</span>
                            <span class="text-xs text-gray-500 font-medium uppercase tracking-wider">Rooms</span>
                        </div>
                    </div>

                    <button data-id="${id}" class="select-building-btn w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl shadow-lg shadow-indigo-200 transition-all duration-200 transform hover:-translate-y-0.5">
                        ${floorCount > 0 ? 'Manage Property' : 'Configure Structure'}
                    </button>
                </div>
            `;
...
// --- LLM API Setup ---
// TODO: Replace with your own API KEY from https://aistudio.google.com/
const API_KEY = "#";
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            ...
            if (response.status === 429 && attempt < MAX_RETRIES - 1) {
                ...
                console.warn(`Gemini 429 hit. Retrying in ${delay / 1000}s...`);
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    // Critical Debugging: If 404, try to list available models to tell the user what IS supported
                    if (response.status === 404) {
                        try {
                            ...
                            if (listData.models) {
                                ...
                                errorMsg += `\n\n[DEBUG] Available Models for your Key: ${modelNames}`;
                            } else {
                                errorMsg += `\n\n[DEBUG] Could not list models (No 'models' field in response).`;
                            }
                        } catch (listErr) {
                            errorMsg += `\n\n[DEBUG] Failed to list models: ${listErr.message}`;
                        }
                    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (backToBuildingsBtn) {
        backToBuildingsBtn.addEventListener('click', () => {
            if (currentBuildingId) {
                // If currently viewing a building, go back to the building list
                currentBuildingId = null;
                showView('BUILDING_CONFIG_VIEW');
            } else {
                // If at the building list, go back to the main dashboard
                showDashboard();
            }
        });
    }
    ...
    function renderMap() {
        ...
        Object.keys(floors).reverse().forEach(floorName => {
            ...
            roomKeys.forEach(roomKey => {
                ...
                roomDiv.className = `room-block w-24 h-24 flex flex-col items-center justify-center p-1 rounded-lg cursor-pointer transition-all duration-200 border-2 text-center ${isOccupied
    ...
    const genActionBtn = document.getElementById('generate-action-plan-btn');
    const genNoticeBtn = document.getElementById('generate-rent-notice-btn');
    const aiOutput = document.getElementById('ai-output');
    const aiLoading = document.getElementById('ai-loading-indicator');
    const aiContainer = document.getElementById('ai-output-container');
    const copyAiBtn = document.getElementById('copy-ai-output');
    ...
    async function handleAiGeneration(type) {
        ...
        if (activeOccupants.length === 0) {
            alert("No active occupants in this room to generate content for.");
            return;
        }

        aiLoading.classList.remove('hidden');
        aiContainer.classList.add('hidden');
        whatsappBtn.classList.add('hidden'); // Hide initially

        let prompt = "";
        let context = `
            Building: ${buildingData[currentBuildingId].name}
            Room: ${roomKey}
            Occupants: ${activeOccupants.map(o => o.name).join(', ')}
            Notes: ${roomData.roomDetails || 'None'}
        `;
        ...
        activeOccupants.forEach(o => {
            ...
            const paid = monthlyRecords.reduce((sum, r) => sum + r.paidAmount, 0);
            totalDue += (expected - paid);
        });
        context += `\nTotal Due for Room: ${CURRENCY_SYMBOL}${totalDue}`;

        if (type === 'plan') {
            prompt = `You are a property manager assistant. Based on the following context, generate a concise, bulleted action plan to manage this room. Focus on financial health and maintenance. Context: ${context}`;
        } else if (type === 'notice') {
            prompt = `You are a polite property manager. Write a short, friendly WhatsApp message to the tenants of this room reminding them of the rent and total due amount. Keep it professional but casual. Include the due amount clearly. Context: ${context}`;
        }
    ...
    if (genActionBtn) genActionBtn.addEventListener('click', () => handleAiGeneration('plan'));
    if (genNoticeBtn) genNoticeBtn.addEventListener('click', () => handleAiGeneration('notice'));
